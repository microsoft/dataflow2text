...
from dataflow2text_domains.calflow.experiments.lispress.program_utils import (
    is_idx_str as is_express_idx_str,
)
from dataflow2text_domains.calflow.experiments.lispress.program_utils import (
    is_struct_op_schema,
    mk_call_op,
...
from dataflow2text_domains.calflow.experiments.lispress.sexp import (
    LEFT_PAREN,
    RIGHT_PAREN,
    Sexp,
    parse_sexp,
...
def _sugar_gets(sexp: Lispress) -> Lispress:
    ...
    if isinstance(sexp, str) or len(sexp) == 0:
        ...
        if hd == DataflowFn.Get.value:
            ...
            obj, path_value = tl
            try:
                _value, (_path, key) = path_value  # type: ignore
    ...
    for expression in program.expressions:
        ...
        if isinstance(expression.op, (BuildStructOp, CallLikeOp)):
            ...
            for i, (arg_name, arg_id) in enumerate(named_args):
                if arg_name is not None and not arg_name.startswith("_arg"):
...
def unnest_line(
    ...
    if not isinstance(s, list):
        ...
        if not isinstance(hd, str):
            ...
            (arg,), body = tl  # type: ignore
            if len(arg) == 1:
                # arg has no type ascription
                arg_type = None
                (arg_name,) = arg  # type: ignore
            else:
                ...
                _, arg_type, arg_name = arg  # type: ignore
            assert isinstance(
                arg_name, str
            ...
            ((schema, *value_tokens),) = tl  # type: ignore
            value = " ".join(value_tokens)  # type: ignore
            try:
                value = loads(value)
            ...
            for arg in tl:
                if isinstance(arg, str) and _is_named_arg(arg):
                    arg_names.append(_named_arg_to_key(arg))
                else:
                    ...
                    args.append(arg_idx)
            call_op, idx = mk_call_op(
                hd, args=args, arg_names=arg_names if arg_names else None, idx=idx
            )
...
def lispress_to_type_name(e: Lispress) -> TypeName:
    ...
    if isinstance(e, str):
        ...
        return TypeName(constructor, tuple([lispress_to_type_name(a) for a in args]))  # type: ignore
    else:
