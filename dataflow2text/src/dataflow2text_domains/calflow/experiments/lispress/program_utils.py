...
from dataflow2text_domains.calflow.experiments.lispress.sexp import Sexp
...
def get_named_args(e: Expression) -> List[Tuple[str, Optional[str]]]:
    ...
    elif isinstance(e.op, CallLikeOp) and e.op.arg_names is not None:
        fields = e.op.arg_names
    else:
        fields = [f"_arg{i}" for i in range(len(e.arg_ids))]
...
def mk_salient_action(idx: Idx) -> Tuple[List[Expression], Idx]:
    ...
    intension_expr, intension_idx = mk_call_op(
        name=DataflowFn.RoleConstraint.value,
        args=[path_idx],
        idx=path_idx,
    )
...
def mk_revise(
    root_location_idx: Idx,
    old_location_idx: Idx,
    new_idx: Idx,
    idx: Idx,
) -> Tuple[Expression, Idx]:
    """
...
def mk_struct_op(
    schema: str,
    args: List[Tuple[Optional[str], Idx]],
    idx: Idx,
    type: Optional[TypeName] = None,  # pylint: disable=redefined-builtin
) -> Tuple[Expression, Idx]:
    new_idx = idx + 1
...
def mk_call_op(
    ...
    flat_exp = Expression(
        id=idx_str(new_idx),
        op=CallLikeOp(name=name, arg_names=arg_names),
        type_args=type_args,
        type=type,
...
def mk_type_name(sexp: Sexp) -> TypeName:
    ...
    return TypeName(hd, tuple([mk_type_name(e) for e in tl]))  # type: ignore
...
def mk_value_op(
    ...
    expr = Expression(id=idx_str(my_idx), op=ValueOp(value=dumped), type=type)
    return expr, my_idx
