...
from clamp.earley.utf8_ranges import Utf8Sequences
...
def re_utf8(s: str) -> NFAFrag[I]:
    return re_concat(
        # NOTE: Need to convert UTF-16 characters to UTF-8 and need to use "surrogatepass" to handle some emoji.
        # See https://stackoverflow.com/questions/54536539/unicodeencodeerror-utf-8-codec-cant-encode-character-ud83d-in-position-38
        *(
            re_atom(c)  # type: ignore
            for c in np.frombuffer(
                s.encode("utf16", "surrogatepass").decode("utf16").encode("utf8"),
                dtype=np.uint8,
            )
        )
    )
...
def re_substring_utf8(s: str, empty_allowed: bool = True) -> NFAFrag[I]:
    ...
    def fragment(end: NFAState[I]) -> NFAState[I]:
        ...
        for code_point_chunk in reversed(code_point_chunks):
            accepts_this_chunk: NFAState = re_sequence(
                np.uint8(x) for x in code_point_chunk
            )(out)
            out = Alternation(is_final=False, next=(accepts_this_chunk, end))
            if empty_allowed:
