...
import numpy as np
from openfst_python import (  # pylint: disable=no-name-in-module
    Arc,
    MutableFst,
    VectorFst,
    Weight,
...
# This should not be set to numpy.uint8 or clamp.util.unit.Unit.
...
class CompiledDFA(CompiledNFA[I]):
    ...
    def is_final_dfa(self, s: int) -> bool:
        """Checks whether we have reached a final (accepting) state."""
        return s >= 0 and self.is_final_array[s]
    ...
    def next_labels(self, s: int) -> Iterable[Union[I, np.uint8]]:
        return self.transition_labels[s] if s >= 0 else []

    def prev_labels(self, s: int) -> Iterable[Union[I, np.uint8]]:
        return self.incoming_labels[s].keys() if s >= 0 else []
...
class Ranges(NFAState[I]):
    ...
    def transition(self, e: Union[I, Unit]) -> Iterable[NFAState[I]]:  # type: ignore[override]
        if not self.applicable(e) or isinstance(e, Unit):  # type: ignore[arg-type]
            return ()
